...
-- Activités
INSERT INTO `Activity` (`ActivyName`, `Description`, `AgeMin`, `AgeMax`, `Capacity`, `dateCreation`, `status`, `typeActivity`) VALUES
...
('2026-03-01 10:00:00', '2026-03-01 12:00:00', 'PRINCIPAL', 'ACTIF',
 (SELECT id FROM `Activity` WHERE `ActivyName`='Atelier Arts'),
 (SELECT id FROM `Animateurs` WHERE nom='Ali' AND prenom='Karim')),
('2026-03-02 14:00:00', '2026-03-02 16:00:00', 'PRINCIPAL', 'ACTIF',
 (SELECT id FROM `Activity` WHERE `ActivyName`='Initiation Musique'),
 (SELECT id FROM `Animateurs` WHERE nom='Ali' AND prenom='Karim')),
('2026-03-03 09:00:00', '2026-03-03 11:00:00', 'PRINCIPAL', 'ACTIF',
 (SELECT id FROM `Activity` WHERE `ActivyName`='Sport collectif'),
 (SELECT id FROM `Animateurs` WHERE nom='Ali' AND prenom='Karim'));

-- Inscriptions
INSERT INTO `Inscription` (`StatusInscription`, `PresenceStatus`, `IncidentNote`, `animation_id`, `enfant_id`) VALUES
('ACTIF', 'NON_SIGNEE', NULL, (SELECT id FROM `Animation` ORDER BY id ASC LIMIT 1),
         (SELECT id FROM `enfant` WHERE prenom='Lina' LIMIT 1)),
('ACTIF', 'NON_SIGNEE', NULL, (SELECT id FROM `Animation` ORDER BY id DESC LIMIT 1),
